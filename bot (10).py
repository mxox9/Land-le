...
# Image URLs
WELCOME_IMAGE = "https://t.me/prooflelo1/601"
SERVICE_IMAGE = "https://t.me/prooflelo1/508"
...
FORCE_JOIN_IMAGE = "https://t.me/prooflelo1/602"  # New force join image
...
# Check if user is member of channel - FIXED
...
def set_user_verified(user_id):
    users_collection.update_one({"user_id": user_id}, {"$set": {"verified": True}})
...
# Start Command with channel join check - FIXED
...
def start_command(message):
    try:
        ...
        # Always send the Welcome + Verify panel (image + caption + inline join & verify)
        caption = "𝐇𝐞𝐲 𝐄𝐋𝐎𝐍𝐀 🫶" + style_text("""
Welcome to GrowOx Bot

📢 Please join our channel to use this bot (click Join), then click Verify below.

🤖 It's A Most Advance Social Marketing Bot Grow Your Social Media With Our Powerful Service Best And Cheapest Price 🤝

🚀 Click Verify to open the Main Menu.
        """)

        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("📢 Join Channel", url=f"https://t.me/prooflelo1"))
        markup.add(InlineKeyboardButton("✅ Verify", callback_data="verify_join"))

        # Use a dedicated force-join image (falls back if missing)
        try:
            bot.send_video(
                chat_id=message.chat.id,
                video=FORCE_JOIN_IMAGE,
                caption=caption,
                reply_markup=markup,
                parse_mode='HTML'
            )
        except Exception as e:
            # fallback to a simple message if sending photo fails
            print(f"Start: failed to send photo: {e}")
            bot.send_message(chat_id=message.chat.id, text=caption, reply_markup=markup, parse_mode='HTML')

    except Exception as e:
        print(f"Start command error (override): {e}")
        try:
...
# Callback Query Handler - FIXED ADMIN BACK BUTTONS

@bot.callback_query_handler(func=lambda call: True)
def callback_handler(call):
    try:
        user_id = call.from_user.id
        # Treat as admin only if caller matches ADMIN_ID
        ...
        # Ensure user exists in DB early
        init_user(user_id)

        # For all actions except verifying membership, ensure user is verified or admin
        if call.data not in ["check_join", "verify_join"] and not is_admin:
            if not is_user_verified(user_id):
                bot.answer_callback_query(call.id, "❌ " + style_text("Please verify/join the channel first."))
                return

        # Navigation: Land (main entry)
        if call.data == "land":
            try:
                # If admin or already verified, show main menu directly
                if is_admin or is_user_verified(user_id):
                    try:
...
"""),
                            ...
                            print(f"Land: failed to send main menu: {e2}")
                else:
                    # Not verified: show verify panel (without calling /start)
                    caption = "👋 " + style_text("""
Welcome to Next Grow Bot
...
""")
                    ...
                    try:
                        bot.edit_message_media(
                            chat_id=call.message.chat.id,
                            message_id=call.message.message_id,
                            media=telebot.types.InputMediaPhoto(media=FORCE_JOIN_IMAGE, caption=caption),
                            reply_markup=markup,
                            parse_mode='HTML'
                        )
                    except Exception as e:
                        try:
                            ...
                            pass
                        try:
                            bot.send_video(chat_id=call.message.chat.id, video=FORCE_JOIN_IMAGE, caption=caption, reply_markup=markup, parse_mode='HTML')
                        except Exception as e2:
                            print(f"Land: failed to send verify panel: {e2}")
            except Exception as e:
                print(f"Land handler error: {e}")
            return
        ...
        if call.data == "admin":
            if is_admin:
                ...
                bot.answer_callback_query(call.id, "🚫 " + style_text("Access denied."))
            return

        # Verification / Check Join — open main menu only after verification
        if call.data in ["check_join", "verify_join"]:
            if is_member(user_id) or is_admin:
                bot.answer_callback_query(call.id, "✅ " + style_text("Thanks for verifying!"))
                # Mark user verified in DB (persist so they don't need to verify again)
                try:
                    set_user_verified(user_id)
                except Exception as _:
                    pass
                # Try to edit the same message to show the main menu (safer UX)
                try:
                    bot.edit_message_media(
                        chat_id=call.message.chat.id,
                        message_id=call.message.message_id,
...
🚀 Start Growing Your Social Media Now!
            """)
                    ...
                    # If editing fails, delete old message and send main menu
                    print(f"Verify: failed to edit message to main menu: {e}")
                    try:
                        bot.delete_message(call.message.chat.id, call.message.message_id)
                    except:
                        pass
...
🚀 Start Growing Your Social Media Now!
            """)
                        ...
                        print(f"Verify: fallback send failed: {e2}")
            else:
        ...
        if call.data.startswith("service_"):
            try:
                ...
                bot.answer_callback_query(call.id, "❌ " + style_text("Invalid service selected."))
            return

        if call.data == "check_txn":
            check_transaction(call)
            return

        # Admin callbacks - use is_admin check (more robust than strict equality)
        # Any admin action requires the caller to be admin (is_admin True)
        ...
        if call.data == "admin_service_ids":
            if is_admin:
        ...
        if call.data == "admin_deduct_balance":
            if is_admin:
        ...
        if call.data.startswith("refill_"):
            order_id = call.data.split("_", 1)[1]
            handle_refill(call, order_id)
            return

        if call.data.startswith("set_price_"):
            if is_admin:
                # set_price_{something}_{service_id} or set_price_{service_id}
                parts = call.data.split("_")
                # handle both variants; pick last numeric segment as service id
        ...
        if call.data.startswith("set_service_id_"):
            if is_admin:
                parts = call.data.split("_")
                for part in reversed(parts):
                    if part.isdigit():
        ...
        # Admin back-buttons: allow admin to return to admin panel
        if call.data in ["admin_balance_back", "admin_prices_back", "admin_service_ids_back", 
                         "admin_users_back", "admin_control_back", "admin_stats_back"]:
            if is_admin:
                try:
                    bot.delete_message(call.message.chat.id, call.message.message_id)
                except:
        ...
        print(f"Callback handler error (override): {e}")
        try:
...
🎯 Choose a category to start ordering:
    ...
    message_id=call.message.message_id,
    media=telebot.types.InputMediaVideo(
        media="https://t.me/prooflelo1/508",
        caption=caption,
        parse_mode='HTML'
    ),
...
# Track Order Feature - FIXED: REMOVED EXTRA MESSAGE
...
🆔 You can find Order ID in your orders history
        """)
        ...
        # Set user state to wait for order ID
...
💳 Cost: ₹{cost if cost != 'Unknown' else 'Unknown'}
            ...
            # Check if refill is available - FIXED: Better refill detection
            if str(rem).lower() in ['true', '1', 'yes'] or rem == True:
...
# Refill Handler - FIXED: Better error handling and response
    ...
    try:
        ...
        if refill_result["success"]:
            ...
            bot.edit_message_text(
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                text=style_text(f"✅ Refill Request Successful!\n\n🆔 Order ID: {order_id}\n\n🔄 Your order has been successfully refilled."),
                reply_markup=InlineKeyboardMarkup().add(
                    InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="land")
                )
            )
            ...
            bot.edit_message_text(
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                text=style_text(f"❌ Refill Failed!\n\n🆔 Order ID: {order_id}\n\n📛 Error: {refill_result['error']}"),
                reply_markup=InlineKeyboardMarkup().add(
                    InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="land")
                )
            )
...
def process_refill(order_id):
    try:
        ...
        if data and data.get("refill") == True:
            return {
                "success": True
            }
        else:
            return {
                "success": False,
                "error": data.get("error", "Refill not available for this order")
            }
...
def admin_balance_control(call):
    try:
        ...
        keyboard.add(InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="admin_balance_back"))
...
def admin_user_control(call):
    try:
        ...
        keyboard.add(InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="admin_users_back"))
...
def admin_manage_prices(call):
    try:
        ...
        keyboard.add(InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="admin_prices_back"))
...
def admin_manage_service_ids(call):
    try:
        ...
        keyboard.add(InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="admin_service_ids_back"))
...
def admin_bot_control(call):
    try:
        ...
        keyboard.add(InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="admin_control_back"))
...
🤖 Bot Status: {'🟢 Online' if bot_enabled else '🔴 Offline'}
        ...
        bot.edit_message_caption(
            ...
            caption=caption,
            reply_markup=InlineKeyboardMarkup().add(
                InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="admin_stats_back")
            ),
            parse_mode='HTML'
        )
    except Exception as e:
...
def handle_unknown(message):
    try:
        ...
        # Check if user is member of channel - FIXED: Always check for non-admin users
