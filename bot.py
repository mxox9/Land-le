...
def style_text(text):
    """Convert text to stylish format for button labels"""
    def style_word(word):
        if len(word) > 0:
...
def services_keyboard(category):
    ...
    for service_name in SERVICES[category].keys():
        markup.add(InlineKeyboardButton(service_name, callback_data=f"serv_{category}_{service_name.replace(' ', '_')}"))
...
def admin_settings_keyboard():
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton(f"🔧 Bot Status: {bot_status}", callback_data="admin_toggle_bot"),
...
💳 Your Balance: ₹{get_balance(user_id):.2f}
    ...
    try:
        ...
        if api_order_id:
            ...
            orders_collection.update_one(
                ...
                {"$set": {"status": "Processing", "api_order_id": api_order_id}}
            )
...
Send a message to all users:
            ...
            msg = bot.send_message(user_id, "📢 Enter your broadcast message:")
...
Configure bot settings:
        ...
        elif call.data == "admin_bot_info":
            show_bot_info(call)
            
        elif call.data == "admin_add_balance":
            admin_add_balance_text = """
➕ Add Balance

Add balance to a user's account.

Please reply with the user ID:
            """
            
            bot.edit_message_text(
                chat_id=user_id,
                message_id=call.message.message_id,
                text=admin_add_balance_text,
                reply_markup=InlineKeyboardMarkup().add(
                    InlineKeyboardButton("🔙 Back", callback_data="admin_balance")
                )
            )
...
Please reply with the user ID:
            ...
            bot.edit_message_text(
                chat_id=user_id,
                message_id=call.message.message_id,
                text=admin_remove_balance_text,
                reply_markup=InlineKeyboardMarkup().add(
                    InlineKeyboardButton("🔙 Back", callback_data="admin_balance")
                )
            )
            ...
            msg = bot.send_message(user_id, "👤 Enter user ID:")
            bot.register_next_step_handler(msg, process_remove_balance_user)
            
        elif call.data == "admin_user_stats":
            show_admin_user_stats(call)
            
        elif call.data == "admin_find_user":
            admin_find_user_text = """
🔍 Find User

Find user by ID or username.

Please reply with user ID or username:
            """
            
            bot.edit_message_text(
                chat_id=user_id,
                message_id=call.message.message_id,
                text=admin_find_user_text,
                reply_markup=InlineKeyboardMarkup().add(
                    InlineKeyboardButton("🔙 Back", callback_data="admin_users")
                )
            )
            
            msg = bot.send_message(user_id, "👤 Enter user ID or username:")
...
Please reply with user ID:
            ...
            admin_unban_user_text = """
✅ Unban User

Unban a user to allow them to use the bot again.
...
Configure bot settings:
        ...
        bot.answer_callback_query(call.id, "❌ Error toggling bot status.", show_alert=True)
...
⚙️ Configuration:
🟢 Bot Status: {'ONLINE' if is_bot_accepting_orders() else 'OFFLINE'}
📱 Services: {sum(len(services) for services in SERVICES.values())}
💳 Payment: AutoDep API
...
👑 Admins: {len(ADMIN_IDS)}
        ...
        print(f"Show bot info error: {e}")
        bot.answer_callback_query(call.id, "❌ Error loading bot info.", show_alert=True)
...
def show_admin_user_stats(call):
    try:
        ...
        # Get recent users
        recent_users = list(users_collection.find().sort("joined_date", -1).limit(10))
...
"""
        ...
        for user in recent_users[:5]:
            user_stats_text += f"\n👤 @{user['username']} | 💰 ₹{user['balance']:.2f} | 📅 {user['joined_date'].strftime('%d/%m')}"
        ...
        bot.answer_callback_query(call.id, "❌ Error loading user statistics.", show_alert=True)
...
def process_broadcast_message(message):
    try:
        ...
        confirm_markup.add(
            InlineKeyboardButton("✅ Send Broadcast", callback_data=f"confirm_broadcast_{message.message_id}"),
        ...
        print(f"Process broadcast message error: {e}")
        bot.send_message(user_id, "❌ Error processing broadcast.", reply_markup=admin_keyboard())
...
def process_add_balance_user(message):
    try:
        ...
        # Save target user ID
        save_user_data(user_id, "admin_target_user", target_user_id)
        
        msg = bot.send_message(user_id, "💰 Enter amount to add:")
        bot.register_next_step_handler(msg, process_add_balance_amount)
        
    except ValueError:
        bot.send_message(user_id, "❌ Invalid user ID. Please enter a valid numeric user ID.")
    except Exception as e:
        print(f"Process add balance user error: {e}")
        bot.send_message(user_id, "❌ Error processing user ID.", reply_markup=admin_balance_keyboard())
...
def process_add_balance_amount(message):
    try:
        ...
        amount = float(message.text)
        target_user_id = get_user_data(user_id, "admin_target_user")
        ...
        # Clear session data
        delete_user_data(user_id, "admin_target_user")
...
💳 New Balance: ₹{new_balance:.2f}
        ...
        bot.send_message(user_id, success_text, reply_markup=admin_balance_keyboard())
        ...
        print(f"Process add balance amount error: {e}")
        bot.send_message(user_id, "❌ Error adding balance.", reply_markup=admin_balance_keyboard())

def process_remove_balance_user(message):
    try:
        user_id = message.from_user.id
        ...
        msg = bot.send_message(user_id, "💰 Enter amount to remove:")
        bot.register_next_step_handler(msg, process_remove_balance_amount)
        
    except ValueError:
        bot.send_message(user_id, "❌ Invalid user ID. Please enter a valid numeric user ID.")
    except Exception as e:
        print(f"Process remove balance user error: {e}")
        bot.send_message(user_id, "❌ Error processing user ID.", reply_markup=admin_balance_keyboard())

def process_remove_balance_amount(message):
    try:
        user_id = message.from_user.id
        ...
        if target_user["balance"] < amount:
            bot.send_message(user_id, f"❌ User only has ₹{target_user['balance']:.2f}, cannot remove ₹{amount:.2f}")
            return
...
💳 New Balance: ₹{new_balance:.2f}
        ...
        try:
            bot.send_message(
                target_user_id,
                f"💰 Admin removed ₹{amount:.2f} from your balance!\n💳 New Balance: ₹{new_balance:.2f}"
            )
        except:
            pass
        ...
        print(f"Process remove balance amount error: {e}")
        bot.send_message(user_id, "❌ Error removing balance.", reply_markup=admin_balance_keyboard())
...
📥 Total Deposits: ₹{user.get('total_deposits', 0):.2f}
📤 Total Spent: ₹{user.get('total_spent', 0):.2f}
📅 Joined: {user['joined_date'].strftime("%d/%m/%Y %H:%M")}
🚫 Banned: {'Yes' if user.get('banned') else 'No'}
        """
        
        markup = InlineKeyboardMarkup()
        if user.get('banned'):
            markup.add(InlineKeyboardButton("✅ Unban User", callback_data=f"admin_unban_{user['user_id']}"))
        else:
            markup.add(InlineKeyboardButton("🚫 Ban User", callback_data=f"admin_ban_{user['user_id']}"))
        
        markup.add(InlineKeyboardButton("🔙 Back", callback_data="admin_users"))
        
        bot.send_message(user_id, user_info, reply_markup=markup)
        
    except Exception as e:
        print(f"Process find user error: {e}")
        bot.send_message(user_id, "❌ Error finding user.", reply_markup=admin_users_keyboard())
...
def process_ban_user(message):
    try:
        ...
        bot.send_message(user_id, f"✅ User {target_user_id} has been banned.", reply_markup=admin_users_keyboard())
        ...
        print(f"Process ban user error: {e}")
        bot.send_message(user_id, "❌ Error banning user.", reply_markup=admin_users_keyboard())
...
def process_unban_user(message):
    try:
        ...
        bot.send_message(user_id, f"✅ User {target_user_id} has been unbanned.", reply_markup=admin_users_keyboard())
        
        # Notify target user
        try:
            bot.send_message(target_user_id, "✅ You have been unbanned and can now use the bot again.")
        except:
            pass
        ...
        print(f"Process unban user error: {e}")
        bot.send_message(user_id, "❌ Error unbanning user.", reply_markup=admin_users_keyboard())
