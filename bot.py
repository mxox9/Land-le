...
def place_smm_order(service_id, link, quantity):
    """Place order via SMM API (made robust to various API responses)."""
    try:
        params = {
            'key': SMM_API_KEY,
            'action': 'add',
        ...
        # Be defensive about the base URL: if it already contains query params, use it as-is.
        url = SMM_API_URL.rstrip('?')
        response = requests.get(url, params=params, timeout=30)
        # Try to parse JSON; if that fails, return None with logging
        try:
            data = response.json()
        except ValueError:
            print(f"SMM API returned non-JSON response: {response.text[:200]!r}")
            return None

        # Commonly APIs return keys like 'order', 'order_id', 'id' or nested structures.
        # Try multiple possibilities in order.
        if isinstance(data, dict):
            # If API explicitly returned an error field, log it and return None
