...
# API Functions - UPDATED ORDER PLACEMENT
...
def place_smm_order(service_id, link, quantity):
    """Place order via SMM API - SIMPLIFIED AND ROBUST"""
    try:
        # Direct API call like in your working example
        api_key = SMM_API_KEY
        url = f"{SMM_API_URL}?key={api_key}&action=add&service={service_id}&link={link}&quantity={quantity}"
        
        print(f"🔗 API URL: {url.replace(api_key, '***')}")  # Log without exposing key
...
# Admin Keyboard Builders - UPDATED WITH STYLISH TEXT
def admin_main_keyboard():
    """Admin main menu keyboard"""
    markup = InlineKeyboardMarkup()
    markup.row_width = 2
    markup.add(
        InlineKeyboardButton(style_text("📱 Manage Services"), callback_data="admin_manage_services"),
        InlineKeyboardButton(style_text("💰 Balance Control"), callback_data="admin_balance_control")
    )
    markup.add(
        InlineKeyboardButton(style_text("👤 User Control"), callback_data="admin_user_control"),
        InlineKeyboardButton(style_text("📢 Broadcast"), callback_data="admin_broadcast")
    )
    markup.add(
        InlineKeyboardButton(style_text("⚙️ Bot Control"), callback_data="admin_bot_control"),
        InlineKeyboardButton(style_text("📊 Stats"), callback_data="admin_stats")
    )
    markup.add(InlineKeyboardButton(style_text("🔙 Main Menu"), callback_data="main_menu"))
    return markup

def admin_services_keyboard():
    """Admin services management keyboard"""
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("➕ Add Service"), callback_data="admin_add_service"))
    markup.add(InlineKeyboardButton(style_text("✏️ Edit Service"), callback_data="admin_edit_service"))
    markup.add(InlineKeyboardButton(style_text("❌ Delete Service"), callback_data="admin_delete_service"))
    markup.add(InlineKeyboardButton(style_text("➕ Add Category"), callback_data="admin_add_category"))
    markup.add(InlineKeyboardButton(style_text("🔙 Admin Menu"), callback_data="admin_menu"))
    return markup

def admin_balance_keyboard():
    """Admin balance control keyboard"""
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("➕ Add Balance"), callback_data="admin_add_balance"))
    markup.add(InlineKeyboardButton(style_text("➖ Deduct Balance"), callback_data="admin_deduct_balance"))
    markup.add(InlineKeyboardButton(style_text("🔙 Admin Menu"), callback_data="admin_menu"))
    return markup
...
def admin_user_keyboard():
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("🔨 Ban User"), callback_data="admin_ban_user"))
    markup.add(InlineKeyboardButton(style_text("✅ Unban User"), callback_data="admin_unban_user"))
    markup.add(InlineKeyboardButton(style_text("🔙 Admin Menu"), callback_data="admin_menu"))
    return markup
...
def admin_bot_control_keyboard():
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("🟢 Turn Bot ON"), callback_data="admin_bot_on"))
    markup.add(InlineKeyboardButton(style_text("🔴 Turn Bot OFF"), callback_data="admin_bot_off"))
    markup.add(InlineKeyboardButton(style_text(f"📊 Current Status: {bot_status}"), callback_data="admin_bot_status"))
    markup.add(InlineKeyboardButton(style_text("🔙 Admin Menu"), callback_data="admin_menu"))
    return markup
...
def service_categories_keyboard():
    ...
    markup.add(InlineKeyboardButton(style_text("➕ Add New Category"), callback_data="admin_add_new_category"))
    markup.add(InlineKeyboardButton(style_text("🔙 Back"), callback_data="admin_manage_services"))
    return markup
...
def services_list_keyboard(category, action):
    ...
    markup.add(InlineKeyboardButton(style_text("🔙 Back"), callback_data=f"admin_{action}_back"))
    return markup

def confirm_delete_keyboard(service_id):
    """Confirmation keyboard for service deletion"""
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("✅ Yes, Delete"), callback_data=f"admin_confirm_delete_{service_id}"))
    markup.add(InlineKeyboardButton(style_text("❌ Cancel"), callback_data="admin_manage_services"))
    return markup

# Main Menu Keyboard Builders - UPDATED WITH STYLISH TEXT
...
def support_keyboard():
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("📞 Contact Us"), url="https://wa.me/639941532149"))
    markup.add(InlineKeyboardButton(style_text("🔙 Back"), callback_data="main_menu"))
    return markup
...
def track_order_keyboard():
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("📊 Track Order"), callback_data="track_order"))
    markup.add(InlineKeyboardButton(style_text("🔙 Back"), callback_data="main_menu"))
    return markup
...
def deposit_confirmation_keyboard():
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("✅ I Have Paid"), callback_data="confirm_deposit"))
    markup.add(InlineKeyboardButton(style_text("❌ Cancel"), callback_data="main_menu"))
    return markup
...
def start_add_category(call):
    ...
    text = style_text("📂 Aᴅᴅ Nᴇᴡ Cᴀᴛᴇɢᴏʀʏ\n\nEɴᴛᴇʀ ᴄᴀᴛᴇɢᴏʀʏ ɴᴀᴍᴇ:")
...
def process_add_category_name(message):
    ...
    if category_name in existing_categories:
        bot.send_message(user_id, style_text(f"❌ Cᴀᴛᴇɢᴏʀʏ '{category_name}' ᴀʟʀᴇᴀᴅʏ ᴇxɪsᴛs!"), reply_markup=admin_main_keyboard())
        del admin_states[user_id]
        return
    
    # Category added successfully
    bot.send_message(user_id, style_text(f"✅ Cᴀᴛᴇɢᴏʀʏ '{category_name}' ᴀᴅᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ!"), reply_markup=admin_main_keyboard())
    del admin_states[user_id]
...
def start_add_new_category(call):
    ...
    text = style_text("📂 Eɴᴛᴇʀ ɴᴇᴡ ᴄᴀᴛᴇɢᴏʀʏ ɴᴀᴍᴇ:")
    bot.send_message(user_id, text)
...
def process_add_service_unit(message):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("❌ Iɴᴠᴀʟɪᴅ ᴜɴɪᴛ! Usᴇ ɴᴜᴍʙᴇʀs ᴏɴʟʏ."))
...
def process_add_service_id(message):
    ...
    bot.send_message(user_id, style_text("🔗 Eɴᴛᴇʀ API URL (ᴏʀ ᴛʏᴘᴇ 'ᴅʙ' ᴛᴏ ᴜsᴇ ᴅᴇғᴀᴜʟᴛ):"))
...
def process_add_service_api_url(message):
    ...
    bot.send_message(user_id, style_text("🔑 Eɴᴛᴇʀ API Kᴇʏ (ᴏʀ ᴛʏᴘᴇ 'ᴅʙ' ᴛᴏ ᴜsᴇ ᴅᴇғᴀᴜʟᴛ):"))
...
🔑 API Kᴇʏ: {('SET' if service_data.get('api_key') else 'GLOBAL')}
    """)
...
def show_service_categories(call, action):
    ...
    try:
        bot.edit_message_text(
            ...
            text=style_text("📱 Sᴇʟᴇᴄᴛ sᴇʀᴠɪᴄᴇ ᴄᴀᴛᴇɢᴏʀʏ:"),
            reply_markup=service_categories_keyboard()
        )
    except:
        bot.send_message(user_id, style_text("📱 Sᴇʟᴇᴄᴛ sᴇʀᴠɪᴄᴇ ᴄᴀᴛᴇɢᴏʀʏ:"), reply_markup=service_categories_keyboard())
...
def show_services_for_edit(call, category):
    ...
    try:
        bot.edit_message_text(
            chat_id=user_id,
            message_id=call.message.message_id,
            text=style_text(f"✏️ Sᴇʟᴇᴄᴛ sᴇʀᴠɪᴄᴇ ᴛᴏ ᴇᴅɪᴛ ({category}):"),
            reply_markup=services_list_keyboard(category, "edit")
        )
    except:
        bot.send_message(user_id, style_text(f"✏️ Sᴇʟᴇᴄᴛ sᴇʀᴠɪᴄᴇ ᴛᴏ ᴇᴅɪᴛ ({category}):"), 
                        reply_markup=services_list_keyboard(category, "edit"))
...
def show_services_for_delete(call, category):
    ...
    try:
        bot.edit_message_text(
            chat_id=user_id,
            message_id=call.message.message_id,
            text=style_text(f"❌ Sᴇʟᴇᴄᴛ sᴇʀᴠɪᴄᴇ ᴛᴏ ᴅᴇʟᴇᴛᴇ ({category}):"),
            reply_markup=services_list_keyboard(category, "delete")
        )
    except:
        bot.send_message(user_id, style_text(f"❌ Sᴇʟᴇᴄᴛ sᴇʀᴠɪᴄᴇ ᴛᴏ ᴅᴇʟᴇᴛᴇ ({category}):"), 
                        reply_markup=services_list_keyboard(category, "delete"))
...
def show_admin_services_menu(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("📱 Sᴇʀᴠɪᴄᴇs Mᴀɴᴀɢᴇᴍᴇɴᴛ\n\nSᴇʟᴇᴄᴛ ᴀɴ ᴀᴄᴛɪᴏɴ:"), 
                        reply_markup=admin_services_keyboard())
...
def show_admin_balance_menu(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("💰 Bᴀʟᴀɴᴄᴇ Cᴏɴᴛʀᴏʟ\n\nSᴇʟᴇᴄᴛ ᴀɴ ᴀᴄᴛɪᴏɴ:"), 
                        reply_markup=admin_balance_keyboard())
...
def show_admin_user_menu(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("👤 Usᴇʀ Cᴏɴᴛʀᴏʟ\n\nSᴇʟᴇᴄᴛ ᴀɴ ᴀᴄᴛɪᴏɴ:"), 
                        reply_markup=admin_user_keyboard())
...
def start_admin_broadcast(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("📢 Bʀᴏᴀᴅᴄᴀsᴛ\n\nSᴇɴᴅ ᴛʜᴇ ᴍᴇssᴀɢᴇ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ʙʀᴏᴀᴅᴄᴀsᴛ ᴛᴏ ᴀʟʟ ᴜsᴇʀs:"))
...
Tʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ sᴇɴᴛ ᴛᴏ ᴀʟʟ ᴜsᴇʀs. Cᴏɴғɪʀᴍ?
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(style_text("✅ Yes, Send"), callback_data="admin_confirm_broadcast"))
    markup.add(InlineKeyboardButton(style_text("❌ Cancel"), callback_data="admin_menu"))
...
def start_add_balance(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("💰 Aᴅᴅ Bᴀʟᴀɴᴄᴇ\n\nSᴇɴᴅ Usᴇʀ ID:"))
...
def start_deduct_balance(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("💰 Dᴇᴅᴜᴄᴛ Bᴀʟᴀɴᴄᴇ\n\nSᴇɴᴅ Usᴇʀ ID:"))
...
def start_ban_user(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("🔨 Bᴀɴ Usᴇʀ\n\nSᴇɴᴅ Usᴇʀ ID ᴛᴏ ʙᴀɴ:"))
...
def start_unban_user(call):
    ...
    try:
        ...
        bot.send_message(user_id, style_text("✅ Uɴʙᴀɴ Usᴇʀ\n\nSᴇɴᴅ Usᴇʀ ID ᴛᴏ ᴜɴʙᴀɴ:"))
        ...
        if message_text:
            ...
            try:
                bot.edit_message_text(
                    chat_id=user_id,
                    message_id=call.message.message_id,
                    text=style_text("📢 Bʀᴏᴀᴅᴄᴀsᴛ sᴇɴᴛ sᴜᴄᴄᴇssғᴜʟʟʏ!"),
                    reply_markup=admin_main_keyboard()
                )
            except:
                bot.send_message(user_id, style_text("📢 Bʀᴏᴀᴅᴄᴀsᴛ sᴇɴᴛ sᴜᴄᴄᴇssғᴜʟʟʏ!"), 
                               reply_markup=admin_main_keyboard())
...
def process_deposit_amount(message):
    ...
    try:
        ...
        if amount < 10:
            bot.send_message(user_id, style_text("❌ Mɪɴɪᴍᴜᴍ ᴅᴇᴘᴏsɪᴛ ᴀᴍᴏᴜɴᴛ ɪs ₹10!"))
            return
...
Aғᴛᴇʀ ᴘᴀʏᴍᴇɴᴛ, ᴄʟɪᴄᴋ "I Hᴀᴠᴇ Pᴀɪᴅ" ʙᴇʟᴏᴡ.
...
📞 Cᴏɴᴛᴀᴄᴛ sᴜᴘᴘᴏʀᴛ ɪғ ʏᴏᴜ ғᴀᴄᴇ ᴀɴʏ ɪssᴜᴇs.
        ...
        bot.send_message(user_id, style_text("❌ Iɴᴠᴀʟɪᴅ ᴀᴍᴏᴜɴᴛ! Pʟᴇᴀsᴇ ᴇɴᴛᴇʀ ᴀ ᴠᴀʟɪᴅ ɴᴜᴍʙᴇʀ."))
...
def process_deposit_confirmation(call):
    ...
    if not payment_verified:
        bot.edit_message_text(
            ...
            text=style_text("❌ Pᴀʏᴍᴇɴᴛ ɴᴏᴛ ᴠᴇʀɪғɪᴇᴅ! Pʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ ᴏʀ ᴄᴏɴᴛᴀᴄᴛ sᴜᴘᴘᴏʀᴛ.")
        )
        return
...
def confirm_order(call):
    """Confirm and process order with API integration (IMPROVED ORDER PLACEMENT)"""
    user_id = call.message.chat.id
    ...
    # Get service API ID (use the service_id from database which should be the SMM panel service ID)
    api_service_id = service.get("service_id") or service.get("smm_service_id") or service["_id"]
    
    # Place order via SMM API - USING SIMPLIFIED METHOD
    api_order_id = place_smm_order(api_service_id, data["link"], data["quantity"])

    if not api_order_id:
        # If order placement failed, provide retry option
        text = style_text("""❌ Fᴀɪʟᴇᴅ ᴛᴏ ᴘʟᴀᴄᴇ ᴏʀᴅᴇʀ!

Pᴏssɪʙʟᴇ ʀᴇᴀsᴏɴs:
• SMM API ʀᴇᴛᴜʀɴᴇᴅ ᴀɴ ᴇʀʀᴏʀ ᴏʀ ᴜɴᴇxᴘᴇᴄᴛᴇᴅ ʀᴇsᴘᴏɴsᴇ.
...
Wᴏᴜʟᴅ ʏᴏᴜ ʟɪᴋᴇ ᴛᴏ ʀᴇᴛʀʏ ᴘʟᴀᴄɪɴɢ ᴛʜᴇ ᴏʀᴅᴇʀ?""")
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(style_text("🔄 Retry"), callback_data="retry_place_order"))
        markup.add(InlineKeyboardButton(style_text("❌ Cancel"), callback_data="order_menu"))
        try:
            bot.edit_message_text(
                chat_id=user_id,
    ...
    # SUCCESS: Order placed via API, deduct balance and create order record
...
⏰ ETA: 24-48 ʜᴏᴜʀs
        ...
        bot.edit_message_text(
            chat_id=user_id,
            message_id=processing_msg.message_id,
            text=style_text("❌ Fᴀɪʟᴇᴅ ᴛᴏ ᴄʀᴇᴀᴛᴇ ᴏʀᴅᴇʀ ɪɴ ᴅᴀᴛᴀʙᴀsᴇ. Aᴅᴍɪɴ ᴡɪʟʟ ʙᴇ ɴᴏᴛɪғɪᴇᴅ.")
        )
...
⚙️ Bᴏᴛ Sᴛᴀᴛᴜs: {'🟢 Oᴘᴇʀᴀᴛɪɴɢ' if is_bot_accepting_orders() else '🔴 Mᴀɪɴᴛᴇɴᴀɴᴄᴇ'}
    """)
...
def start_track_order(call):
    ...
    bot.send_message(user_id, style_text("🔍 Eɴᴛᴇʀ ʏᴏᴜʀ Oʀᴅᴇʀ ID:"))
...
def process_track_order(message):
    ...
    if not order or order["user_id"] != user_id:
        bot.send_message(user_id, style_text("❌ Oʀᴅᴇʀ ɴᴏᴛ ғᴏᴜɴᴅ ᴏʀ ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴘᴇʀᴍɪssɪᴏɴ ᴛᴏ ᴠɪᴇᴡ ᴛʜɪs ᴏʀᴅᴇʀ!"))
        del user_states[user_id]
        return
