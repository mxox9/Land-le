...
def verify_deposit(call):
    ...
    try:
        # AutoDep API verification
        api_url = f"https://erox-autodep-api.onrender.com/api?key={AUTODEP_API_KEY}&merchantkey={MERCHANT_KEY}&transactionid={utr}"
...
Cᴏᴍᴍᴀɴᴅs:
/addservice - Aᴅᴅ ɴᴇᴡ sᴇʀᴠɪᴄᴇ
/editservice - Eᴅɪᴛ sᴇʀᴠɪᴄᴇ
    ...
    while True:
        try:
            ...
            for order in orders:
                ...
                # In production, you would call your SMM provider's API here
                status = check_order_status(order)  # Implement this function
...
def check_order_status(order):
    ...
    # Example API call:
    # response = requests.get(f"{SMM_API_URL}/status?key={SMM_API_KEY}&action=status&order={order['external_id']}")
...
def calculate_refund(order, status):
    ...
    if status == "Cancelled":
        ...
        # Partial refund - implement your logic
