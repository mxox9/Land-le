...
# Image URLs
WELCOME_IMAGE = "https://t.me/prooflelo1/485"
...
def get_services():
    return {
        "instagram": {
            ...
            "services": {
                1: {"name": "❤️ " + style_text("Instagram Likes"), "rate": service_prices.get(1, 50), "min": 100, "max": 100000, "unit": 1000, "api_id": 1},
                2: {"name": "👀 " + style_text("Instagram Views"), "rate": service_prices.get(2, 1), "min": 100, "max": 100000, "unit": 1000, "api_id": 13685},
                3: {"name": "👥 " + style_text("Instagram Followers"), "rate": service_prices.get(3, 100), "min": 50, "max": 50000, "unit": 1000, "api_id": 3}
            }
        },
        "facebook": {
            "name": "📘 " + style_text("Facebook"), 
            "services": {
                4: {"name": "👍 " + style_text("Facebook Likes"), "rate": service_prices.get(4, 40), "min": 100, "max": 100000, "unit": 1000, "api_id": 4},
                5: {"name": "👀 " + style_text("Facebook Views"), "rate": service_prices.get(5, 35), "min": 100, "max": 100000, "unit": 1000, "api_id": 5},
                6: {"name": "👥 " + style_text("Facebook Followers"), "rate": service_prices.get(6, 80), "min": 50, "max": 50000, "unit": 1000, "api_id": 6}
            }
        },
        "youtube": {
            "name": "📺 " + style_text("YouTube"),
            "services": {
                7: {"name": "👍 " + style_text("YouTube Likes"), "rate": service_prices.get(7, 60), "min": 100, "max": 100000, "unit": 1000, "api_id": 7},
                8: {"name": "👀 " + style_text("YouTube Views"), "rate": service_prices.get(8, 45), "min": 100, "max": 100000, "unit": 1000, "api_id": 8},
                9: {"name": "🔔 " + style_text("YouTube Subscribers"), "rate": service_prices.get(9, 150), "min": 50, "max": 25000, "unit": 1000, "api_id": 9}
            }
        },
        "telegram": {
            "name": "✈️ " + style_text("Telegram"),
            "services": {
                10: {"name": "👥 " + style_text("Telegram Members"), "rate": service_prices.get(10, 200), "min": 50, "max": 10000, "unit": 1000, "api_id": 10},
                11: {"name": "❤️ " + style_text("Telegram Post Likes"), "rate": service_prices.get(11, 80), "min": 100, "max": 50000, "unit": 1000, "api_id": 11},
                12: {"name": "👀 " + style_text("Telegram Post Views"), "rate": service_prices.get(12, 50), "min": 100, "max": 50000, "unit": 1000, "api_id": 12}
            }
        }
    }
...
def main_menu_keyboard():
    ...
    buttons = [
        ...
        InlineKeyboardButton("❓ " + style_text("How To Use"), callback_data="how_to_use"),
...
def show_how_to_use(call):
    try:
        ...
        print(f"How to use error: {e}")
        bot.answer_callback_query(call.id, "❌ " + style_text("Error showing how to use. Please try again."))
...
# Track Order Feature - Fixed to check API directly
...
def handle_track_order_id(message):
    ...
    try:
        # Check order status via SMM API directly (not database)
        status_result = check_order_status(order_id)
        
        if status_result["success"]:
            status = status_result["status"]
            refill_available = status_result.get("refill_available", False)
...
🔗 Link: {link}
📊 Quantity: {quantity}
💳 Cost: ₹{cost if cost != 'Unknown' else 'Unknown':.2f}
📊 Status: {status}
            """)
...
# Orders History - Fixed to show user's own orders
...
# Admin Functions with inline buttons - Fixed all callbacks
def admin_balance_control(call):
    try:
        ...
        keyboard.add(InlineKeyboardButton("🔙 " + style_text("Back"), callback_data="admin_keyboard"))
