...
BOT_TOKEN = os.getenv('BOT_TOKEN', '8052955693:AAGIc2IEETmgTo-MR0czvEhH9nq0QXbv2jc')
...
def place_smm_order(service_id, link, quantity):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get('order'):
                return data['order']  # Return API order ID
        return None
    except Exception as e:
...
💎 I'ᴍ ᴀɴ SMM Sᴇʀᴠɪᴄᴇs ʙᴏᴛ. I ᴄᴀɴ ʜᴇʟᴘ ʏᴏᴜ ɢᴇᴛ ʀᴇᴀʟ sᴏᴄɪᴀʟ ᴍᴇᴅɪᴀ ᴇɴɢᴀɢᴇᴍᴇɴᴛ ᴀᴛ ᴛʜᴇ ʙᴇsᴛ ᴘʀɪᴄᴇs.

⭐ Fᴇᴀᴛᴜʀᴇs:
• Iɴsᴛᴀɢʀᴀᴍ Fᴏʟʟᴏᴡᴇʀs/Lɪᴋᴇs
• YᴏᴜTᴜʙᴇ Vɪᴇᴡs/Sᴜʙsᴄʀɪʙᴇʀs  
• Tᴇʟᴇɢʀᴀᴍ Mᴇᴍʙᴇʀs/Rᴇᴀᴄᴛɪᴏɴs
• Fᴀᴄᴇʙᴏᴏᴋ Lɪᴋᴇs/Fᴏʟʟᴏᴡᴇʀs
• Aɴᴅ ᴍᴀɴʏ ᴍᴏʀᴇ!
...
def confirm_order(call):
    """Confirm and process order with API integration"""
    user_id = call.message.chat.id
    ...
    if not api_order_id:
        bot.edit_message_text(
            ...
            text="❌ Fᴀɪʟᴇᴅ ᴛᴏ ᴘʟᴀᴄᴇ ᴏʀᴅᴇʀ! Pʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ."
        )
        del user_states[user_id]
        return
    ...
    order = create_order(user_id, data["service_id"], data["link"], data["quantity"], data["cost_points"], api_order_id)
