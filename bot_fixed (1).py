...
⚙️ BOT_FIXED.PY
====================
✅ This version does NOT use MongoDB for loading services.
✅ All services are loaded locally from the `default_services` dictionary.
✅ Category not found issue fixed.
✅ Rest of bot features (deposit, admin, orders, refund) unchanged.
"""
...
def init_services():
    pass  # Disabled MongoDB service init
    default_services = {
        "instagram": {
...
def get_services():
    return default_services  # MongoDB disabled, using local data
...
def update_service_disabled(category, service_id, new_data):
    services_data = default_services
...
def callback_handler(call):
    ...
    try:
        ...
        if call.data == "land":
            ...
            bot_enabled = True
            bot.answer_callback_query(call.id, "🟢 " + style_text("Bot has been enabled"))
            admin_bot_control(call)
