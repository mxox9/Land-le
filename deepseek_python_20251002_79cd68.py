...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS user_sessions
                     ...
                     order_category TEXT, order_service_name TEXT, 
                     order_service_data TEXT, order_link TEXT)''')
...
def main_menu_keyboard():
    ...
    if ADMIN_ID in ADMIN_IDS:
        keyboard.add(types.InlineKeyboardButton("👑 Aᴅᴍɪɴ", callback_data="admin"))
    return keyboard
...
🚀 𝔹𝕦𝕪 𝕊𝕠𝕔𝕚𝕒𝕝 𝕄𝕖𝕕𝕚𝕒 𝕊𝕖𝕣𝕧𝕚𝕔𝕖𝕤 𝕒𝕥 ℂ𝕙𝕖𝕒𝕡𝕖𝕤𝕥 ℝ𝕒𝕞𝕖𝕤!
...
# Callback query handler - FIXED ALL BUTTONS
            ...
            try:
                ...
                if resp.get("result", {}).get("STATUS") == "TXN_SUCCESS":
                    ...
                    bot.answer_callback_query(
                        ...
                        text="❌ You have not deposited yet. Please pay first.",
                        show_alert=True
                    )

            except Exception as api_error:
                bot.answer_callback_query(callback_query_id=call.id, text="❌ API error occurred.", show_alert=True)

        else:
            bot.answer_callback_query(callback_query_id=call.id, text="⚠️ No pending deposit found.", show_alert=True)

    except Exception as e:
        bot.answer_callback_query(callback_query_id=call.id, text="❌ An error occurred.", show_alert=True)
...
def show_categories(user_id, message_id):
    try:
        bot.edit_message_media(
            ...
            media=types.InputMediaPhoto(
                media=SERVICE_IMAGE,
                caption="🎯 *Select a Category*\n\nChoose a category to view services:"
            ),
...
def show_services(user_id, message_id, category):
    if category not in SERVICES:
        bot.answer_callback_query(message_id, "❌ Category not found!", show_alert=True)
        return
...
def start_order_flow(user_id, message_id, category, service_name):
    if category not in SERVICES or service_name not in SERVICES[category]:
        bot.answer_callback_query(message_id, "❌ Service not found!", show_alert=True)
        return
...
🎁 Start referring and earn more!
    ...
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton("📤 Share Link", url=f"tg://msg_url?url={quote(refer_link)}"))
    keyboard.add(types.InlineKeyboardButton("🔙 Bᴀᴄᴋ", callback_data="land"))
...
📈 Detailed analytics coming soon!
    ...
    try:
        ...
        bot.send_message(user_id, admin_stats_text, parse_mode='Markdown')
