...
from database import (
    ...
    get_order_by_id, is_bot_accepting_orders
)
...
def setup_user_handlers(bot):
    ...
    def send_welcome(message):
        ...
        if not check_channel_membership(bot, user_id, channel_id):
            text = style_text(f"""
👋 Wᴇʟᴄᴏᴍᴇ {user_name}!
...
Aғᴛᴇʀ ᴘᴀʏᴍᴇɴᴛ, ᴄʟɪᴄᴋ "I Hᴀᴠᴇ Pᴀɪᴅ".
        """)
        
        from keyboards import deposit_confirmation_keyboard
        bot.send_photo(user_id, payment_data["qr_code"], caption=text, 
    ...
    def process_deposit_confirmation(bot, call):
        ...
        payment_verified = True  # verify_payment(transaction_id)
...
💸 100 ᴘᴏɪɴᴛs = ₹1
    ...
    def check_channel_join(bot, call):
        ...
        if check_channel_membership(bot, user_id, channel_id):
            send_welcome(call.message)
        else:
            bot.answer_callback_query(call.id, style_text("❌ Pʟᴇᴀsᴇ ᴊᴏɪɴ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ ғɪʀsᴛ!"), show_alert=True)

    def check_channel_membership(bot, user_id, channel_id):
